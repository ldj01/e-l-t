...
void usage ()
{
    ...
    printf ("    -qa_type: LEDAPS RADSAT=%d, LEDAPS CLOUD=%d, "
            "LASRC AEROSOL=%d, LASRC_RADSAT=%d\n", LEDAPS_RADSAT, LEDAPS_CLOUD,
            LASRC_AEROSOL, LASRC_RADSAT);
...
)
{
    ...
    *qa_type = -99;  /* initialize the QA type */
    ...
    if (*qa_type < LEDAPS_RADSAT || *qa_type > LASRC_RADSAT)
    {
...
int main (int argc, char** argv)
{
    ...
    uint16_t *lasrc_radsat_qa = NULL;  /* Level-2 LaSRC RADSAT QA band */
        ...
        qa_type == LASRC_AEROSOL)
    {
        ...
        if (read_level2_qa (fp_l2qa, nlines, nsamps, qa_type, level2_qa)
            != SUCCESS)
        {  /* Error messages already written */
            exit (EXIT_FAILURE);
        }
    ...
    {
        /* Allocate memory for the entire Level-2 QA band */
        lasrc_radsat_qa = calloc (nlines*nsamps, sizeof (uint16_t));
        if (lasrc_radsat_qa == NULL)
        {
            sprintf (errmsg, "Allocating memory for the Level-2 LaSRC RADSAT "
                "QA band");
            error_handler (true, FUNC_NAME, errmsg);
            return (ERROR);
        }
        ...
        /* Read the entire band of Level-2 QA data */
        if (read_level2_qa (fp_l2qa, nlines, nsamps, qa_type, lasrc_radsat_qa)
            != SUCCESS)
        {  /* Error messages already written */
            exit (EXIT_FAILURE);
        }
        ...
        /* Print out the desired pixel values for testing */
        printf ("Pixel line 0, sample 0: %d\n", lasrc_radsat_qa[0]);
        printf ("Pixel line 0, sample 1000: %d\n", lasrc_radsat_qa[1000]);
        printf ("Pixel line 2029, sample 1749: %d\n",
                lasrc_radsat_qa[2030*nsamps+1749]);
        printf ("Pixel line 2042, sample 1737: %d\n",
                lasrc_radsat_qa[2042*nsamps+1737]);
        printf ("Pixel line 2029, sample 1747: %d\n",
                lasrc_radsat_qa[2029*nsamps+1747]);
        printf ("Pixel line 775, sample 3468: %d\n",
                lasrc_radsat_qa[775*nsamps+3468]);
        printf ("Pixel line 2335, sample 4403: %d\n",
                lasrc_radsat_qa[2335*nsamps+4403]);

        /* Test the pixel interrogation for LASRC RADSAT */
        if (qa_type == LASRC_RADSAT)
        {
            printf ("Value 1: ");   /* LASRC fill */
            if (lasrc_radsat_qa_is_fill (1))
                printf ("**Fill\n");
            else
                printf ("Not fill\n");

            printf ("Value 0: ");   /* LASRC not fill */
            if (lasrc_radsat_qa_is_fill (0))
                printf ("Fill\n");
            else
                printf ("**Not fill\n");

            printf ("Value 2: ");   /* LASRC band 1 saturated */
            if (lasrc_radsat_qa_is_saturated (2, LASRC_B1_SAT_BIT))
                printf ("**Band 1 Saturated\n");
            else
                printf ("Band 1 Not saturated\n");

            printf ("Value 4: ");   /* LASRC band 2 saturated */
            if (lasrc_radsat_qa_is_saturated (4, LASRC_B2_SAT_BIT))
                printf ("**Band 2 Saturated\n");
            else
                printf ("Band 2 Not saturated\n");

            printf ("Value 8: ");   /* LASRC band 3 saturated */
            if (lasrc_radsat_qa_is_saturated (8, LASRC_B3_SAT_BIT))
                printf ("**Band 3 Saturated\n");
            else
                printf ("Band 3 Not saturated\n");

            printf ("Value 16: ");   /* LASRC band 4 saturated */
            if (lasrc_radsat_qa_is_saturated (16, LASRC_B4_SAT_BIT))
                printf ("**Band 4 Saturated\n");
            else
                printf ("Band 4 Not saturated\n");

            printf ("Value 32: ");   /* LASRC band 5 saturated */
            if (lasrc_radsat_qa_is_saturated (32, LASRC_B5_SAT_BIT))
                printf ("**Band 5 Saturated\n");
            else
                printf ("Band 5 Not saturated\n");

            printf ("Value 64: ");   /* LASRC band 6 saturated */
            if (lasrc_radsat_qa_is_saturated (64, LASRC_B6_SAT_BIT))
                printf ("**Band 6 Saturated\n");
            else
                printf ("Band 6 Not saturated\n");

            printf ("Value 128: ");   /* LASRC band 7 saturated */
            if (lasrc_radsat_qa_is_saturated (128, LASRC_B7_SAT_BIT))
                printf ("**Band 7 Saturated\n");
            else
                printf ("Band 7 Not saturated\n");

            printf ("Value 256: ");   /* LASRC band 8 saturated */
            if (lasrc_radsat_qa_is_saturated (256, LASRC_B8_SAT_BIT))
                printf ("**Band 8 Saturated\n");
            else
                printf ("Band 8 Not saturated\n");

            printf ("Value 512: ");   /* LASRC band 9 saturated */
            if (lasrc_radsat_qa_is_saturated (512, LASRC_B9_SAT_BIT))
                printf ("**Band 9 Saturated\n");
            else
                printf ("Band 9 Not saturated\n");

            printf ("Value 1024: ");   /* LASRC band 10 saturated */
            if (lasrc_radsat_qa_is_saturated (1024, LASRC_B10_SAT_BIT))
                printf ("**Band 10 Saturated\n");
            else
                printf ("Band 10 Not saturated\n");

            printf ("Value 2048: ");   /* LASRC band 11 saturated */
            if (lasrc_radsat_qa_is_saturated (2048, LASRC_B11_SAT_BIT))
                printf ("**Band 11 Saturated\n");
            else
                printf ("Band 11 Not saturated\n");

            printf ("Value 128: ");   /* LASRC band 1 not saturated */
            if (lasrc_radsat_qa_is_saturated (128, LASRC_B1_SAT_BIT))
                printf ("Band 1 Saturated\n");
            else
                printf ("**Band 1 Not saturated\n");
        }
